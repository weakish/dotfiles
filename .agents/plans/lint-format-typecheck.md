...
| **checkbashisms** | Dropped — apt only via heavy `devscripts` (Debian/Ubuntu); dual ShellCheck covers it |
...
| **beautysh** | Reject — Bash-oriented; wrong dialect for POSIX `#!/bin/sh`, less common in CI |
...
Shell system packages locally and in CI: `shellcheck`, `shfmt`. Locally, `ruff`, `shellcheck`, and `shfmt` are available via MacPorts; `ty` is on Homebrew (see Brewfile), not MacPorts. **shuck** is not on MacPorts or Homebrew — only via `requirements-dev.txt` (PyPI `shuck-cli`; Linux/macOS arm64 wheels — no macOS x86_64 wheel as of 0.0.45).
...
2. Ensure `myconfig.py` exists so `dodo.py`’s `import myconfig` resolves under `ty check` — copy only when missing (fail loud on `cp` error; no `2>/dev/null || true`):
   ...
   ```
3. Require `ruff`, `ty`, `shuck`, `shellcheck`, `shfmt` (install hints; no auto-install). Missing `ruff`/`ty`/`shuck` → `pip install -r requirements-dev.txt` (`ruff` also via MacPorts; `ty` via Homebrew; `shuck` pip-only — not MacPorts/Homebrew); shell lint hint prefers MacPorts then Homebrew (`command -v`).
4. Run in order:
   - `ruff check .`
   - `ruff format --check .`
   - `ty check`
...
- Ruby / Lua app config (e.g. [`get_authorized_signers.rb`](../../get_authorized_signers.rb), [`.hammerspoon/init.lua`](../../.hammerspoon/init.lua)) — not product code for this lint pass; also pre-commit, pip lockfile.
- Declaring `ruff` / `shellcheck` / `shfmt` in [`Brewfile`](../../Brewfile) — optional follow-up (Homebrew). MacPorts provides `ruff`, `shellcheck`, and `shfmt`; `ty` is Homebrew-only (in Brewfile). [`ports.txt`](../../ports.txt) is regenerated by setup. `script/check` missing-tool hints still cover local install.
...
- [x] Add `ty` to [`Brewfile`](../../Brewfile) (Homebrew; not in MacPorts)
